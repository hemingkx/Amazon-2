...
from data_loader import AmazonDataset
...
def test():
    ...
    test_dataset = AmazonDataset(word_test, label_test, config)
    ...
    if config.model_dir is not None:
        model = BertForCls.from_pretrained(config.model_dir)
    ...
    logging.info("test loss: {}, accuracy: {}".format(val_metrics['loss'], val_acc))
...
def run():
    ...
    # build dataset
    train_dataset = AmazonDataset(word_train, label_train, config)
    dev_dataset = AmazonDataset(word_dev, label_dev, config)
    logging.info("--------Dataset Build!--------")
    # get dataset size
    ...
    model = BertForCls.from_pretrained(config.bert_model, num_labels=2)
    model.to(device)
    # Prepare optimizer
